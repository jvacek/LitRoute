...
flamerelay/static/js/
  pages/        Route-level screens. `pages/<Page>.tsx` for single-file pages,
                ...
                lives in `pages/<Page>.loader.ts(x)` next to the page.
...
- **Route loaders** (`pages/<Page>.loader.ts(x)`) for initial-paint data — see _Data loading (route loaders)_.
- **`useState` / `useReducer`** for local component state, including server data updated after mutations.
- **`AuthContext`** (via `useAuth()`) for `isAuthenticated` / `username` / `name`.
...
import {
  ...
  primaryBtnMd,
  primaryBtnSm,
  primaryBtn, // amber fill, text-white — primaryBtn adds w-full
  amberCharBtnLg,
  amberCharBtnMd, // amber fill, text-char — landing CTAs and Unit hero
  outlineOnDarkBtnMd, // translucent-white outline — Unit hero on dark bg
...
Steps: `email → code → (name?) → app` (passkey users skip the code step via autofill — see below)
